...
async function get_frames() {
  ...
  volcano_frames.sort(function(a, b) {return b['mtime'] - a['mtime'];});
  ...
  for (var i = 0; i < volcano_frames.length; i++) {
    /* get frame id: */
    var frame_id = volcano_frames[i]['id'];
    /* init plot var for this frame id: */
    if (plot_vars[frame_id] == undefined) {
      plot_vars[frame_id] = {};
    };
    /* data file name for this frame id: */
    var data_file = plot_vars['data_prefix'] + '/' + volcano_region + '/' +
                    volcano_name + '_' + frame_id + '/' +
...
async function plot_data() {
  ...
  var ifg_data_file = volcano_region + '/' + volcano_name + '_' + frame_id +
                      '/' + plot_vars['data_dir'] + '/' + ifg_date +
                      '.json.gz';
  /* load the data: */
...
async function load_page() {
  ...
  /* data file to load: */
  var data_file = volcano_region + '/' + volcano_name + '_' + frame_id + '/' +
                  plot_vars['data_dir'] + '/licsalert_data.json.gz';
  /* load the data: */
